...
int32_t confparser_serialize_tnt_config(uint8_t *buffer, const tnt_config *conf) {
	...
	buffer_append_float16(buffer, conf->yaw_filter, 10, &ind);
	...
	buffer_append_uint16(buffer, conf->tc_braking_off_time, &ind);
...
bool confparser_deserialize_tnt_config(const uint8_t *buffer, tnt_config *conf) {
	...
	conf->yaw_filter = buffer_get_float16(buffer, 10, &ind);
	...
	conf->tc_braking_off_time = buffer_get_uint16(buffer, &ind);
...
void confparser_set_defaults_tnt_config(tnt_config *conf) {
	...
	conf->yaw_filter = APPCONF_TNT_YAW_FILTER;
