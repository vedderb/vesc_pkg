...
int32_t confparser_serialize_tnt_config(uint8_t *buffer, const tnt_config *conf) {
	...
	buffer_append_float16(buffer, conf->pitch_gyro_filter, 10, &ind);
	...
	buffer[ind++] = (uint8_t)conf->pitch_ema_factor;
...
bool confparser_deserialize_tnt_config(const uint8_t *buffer, tnt_config *conf) {
	...
	conf->pitch_gyro_filter = buffer_get_float16(buffer, 10, &ind);
	...
	conf->pitch_ema_factor = buffer[ind++];
