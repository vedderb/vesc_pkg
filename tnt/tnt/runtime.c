...
void runtime_data_update(RuntimeData *rt) {
	...
	rt->gyro_y = cosf(roll_rad) * cosf(roll_rad) * rt->gyro[1] + cosf(roll_rad) * sinf(roll_rad) * rt->gyro[2]; //rt->gyro_y = cosf(roll_rad) * gyro[1] + sinf(roll_rad) * gyro[2];
	rt->gyro_z = cosf(roll_rad) * sinf(roll_rad) * rt->gyro[2];
