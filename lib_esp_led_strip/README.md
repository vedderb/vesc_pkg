...
The package includes a VESC Tool page for testing. Each LED strip is a tab: use **+ Add** to add a strip, set its pin / LED count / type / timing, then play with effects, palettes, colour, per-strip level and RGBW auto-white (enabled only on strips with a white channel). Several strips on different pins run at once (the firmware pools the chip's RMT channels behind the pins). A **Global** tab holds master brightness, fade and the current limit, plus **Stop all**. The controls send LispBM expressions to the device as custom app data, which `esp_led_strip.lisp` evaluates. One strip is added automatically when the page opens.
...
| `ext-esp_led-seg-custom` | `(i)` | put segment `i` in custom mode (effect 1): a blank consumer-driven pixel buffer, so you can run your own effect while still getting brightness / current limit / channel pooling |
...
- The current limit (`ext-esp_led-ablimit`) is an estimate (~20 mA per full channel + 1 mA idle per LED at 5 V), summed across all segments. The scale it produces is computed from the previous frame's demand, so a sudden jump can exceed the cap for one frame (~33 ms) before settling.
- Overlay pixels deliberately bypass master brightness, fading and the current limiter - they are meant for headlights that must not dim with the effects. Turning a segment off (`ext-esp_led-seg-on i 0`) blanks its overlay pixels too.
