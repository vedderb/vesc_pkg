...
(defun config-scd () {
        ...
        ; (bms-subcmd-cmdonly 1 0x001F) ; CHGTEST()
        ; (bms-subcmd-cmdonly 1 0x0020) ; DSGTEST()
...
(defun init-hw () {
        (loopwhile (not (bms-init (bms-get-param 'cells_ic1))) {
                (bms-subcmd-cmdonly 1 0x000e)
                (bms-subcmd-cmdonly 1 0x000e)
                (sleep 1)
        })
        ...
        (setq psw-state false) ; Init will disable the power switch on this hw
})
...
(defunret test-chg (samples) {
        ; Disable charge detection when drawing a lot of current as it isn't always accurate then
        (if (and psw-state (< (secs-since current-ts) 3))
            (return false)
        )
...
(defunret psw-on () {
        ...
        (var vdiff (fn () (- (assoc rtc-val 'v-tot) (bms-get-vout))))

        (bms-set-pchg 1)

        (sleep 0.3) ; Wait for some ADC samples to be taken
...
(defun start-fun () {
        ...
        ; Every 50 startupts we do some additional checks. That allows
        ; adjusting the sleep time based on the SoC.
        (if (= (mod (assoc rtc-val 'wakeup-cnt) 50) 0) {
        ...
        ; We rely on the BQ to detect the charger on this hardware, so this check has to be done after
        ; init-hw
        ...
        (var ichg 0.0)
        (if (and charge-ok (not do-sleep) (or charge-wakeup (test-chg 50))) {
                (set-chg true)
...
(defun main-ctrl () (loopwhile t {
            ...
            (setq main-state "Get VOUT")
            (setq vout (bms-get-vout))
            (setq main-state "Get VCHG")
            (setq vt-vchg (bms-get-vchg))
            (setq main-state "Get Current")
            (setq iout (bms-current))

            (if (> (abs iout) 20.0) (setq current-ts (systime)))

            (setq main-state "Get Bal")
            ...
            (if (and (or is-charging (test-chg 1)) charge-ok)
                {
                    (setq main-state "Set CHG")
            ...
            (setq main-state "Check Sleep Off")
            ...
            (setq main-state "Check Sleep SOC")
...
(defun main () {
	(set-fw-name "harmony")
        ...
        (def cell-num (+ (bms-get-param 'cells_ic1)))
