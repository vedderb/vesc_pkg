...
# Replace version and add build timestamp
sed "s/\(readonly property string const_ebike_VERSION: \"\)<unknown>\(\"\)/\1${FULL_VERSION}\2/" "$DIST_UI_FILE" > "$DIST_UI_FILE.tmp"
sed "s/\(readonly property string const_ebike_RELEASE_DATE: \"\)<unknown>\(\"\)/\1${BUILD_TIMESTAMP}\2/" "$DIST_UI_FILE.tmp" > "$DIST_UI_FILE"
